...
为确保前后端交互的一致性，所有API响应必须遵循以下统一格式：

```java
public class ApiResponse<T> {
    private int code;           // 响应状态码，200表示成功，其他表示错误
    private T data;             // 实际数据，成功时包含，失败时为null
    private String message;     // 响应消息，描述操作结果
    private String timestamp;   // 时间戳，ISO 8601格式
}
```
...
1. **响应结构**：所有API响应都必须使用`ApiResponse<T>`包装类
2. **状态码**：
   - `200`: 成功
   - `400`: 客户端错误（如验证失败）
   - `401`: 未授权
...
3. **数据字段**：
   ...
   - `message`: 必须存在，提供用户友好的错误信息
   - `timestamp`: 必须存在，使用ISO 8601格式（LocalDateTime.toString()）
4. **创建方式**：
   - 使用`ApiResponse.success(data, message)`创建成功响应
   - 使用`ApiResponse.error(message, code)`创建错误响应
5. **前端处理**：前端通过检查`code`字段判断请求状态，通过`data`字段获取实际数据

> 💡 **重要提示**：禁止直接返回原始数据或使用自定义响应格式。必须始终使用`ApiResponse<T>`包装类以确保前后端交互的一致性。
